...
const MapComponent = () => {
  ...
  const createCustomClusterIcon = (cluster, type) => {
    ...
    return L.divIcon({
      ...
      <div class="flex flex-col items-center justify-center w-full h-full rounded-lg" style="background-color: ${color}">
        <span style = "font-weight: bold; color: #fff; font-size: "14px">${count}</span>
        ${ type === "Places" ? `<img src =${icon} class="w-full h-full"/>` : icon}
      <div />
      </div>
      `,
